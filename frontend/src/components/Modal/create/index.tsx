...
const CreateModal: React.FC<CreateModalProps> = ({ nav, activeNav, open, onOpenChange, isMobile }) => {
    const [file, setFile] = useState<File | null>(null);
    const [previewUrl, setPreviewUrl] = useState<string | null>(null);
    const [caption, setCaption] = useState<string>(""); // Added caption state
    const inputRef = useRef<HTMLInputElement | null>(null);

    const dispatch = useDispatch(); // If you have a typed AppDispatch, use: useDispatch<AppDispatch>()
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <DialogContent className="xl:w-[40%] h-[90%] sm:w-[90%] w-[95%] ">
                ...
                {!file && (
                    <div className="flex flex-col space-y-4 sm:space-y-6 justify-center items-center text-sm sm:text-base md:text-lg text-neutral-900 dark:text-white min-h-[150px] sm:min-h-[200px] md:min-h-[250px]">
                        <MediaIcon size={60} />
                            ...
                            <>
                                <div className="flex flex-col justify-center space-y-3 sm:space-y-4 w-full">
                                    {previewUrl && file.type.startsWith("image/") && (
                                        <div className="max-w-[250px] max-h-[200px] sm:max-w-[280px] sm:max-h-[280px] md:max-w-[20rem] md:max-h-[20rem] object-contain mx-auto relative" style={{ width: '100%', height: '200px' }}>
