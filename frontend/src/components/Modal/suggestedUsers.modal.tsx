...
const SuggestedUsersModal: React.FC<SuggestedUsersModalProps> = ({
  isOpen,
  onClose,
  suggestedUsers,
  onFollow,
      ...
      <DialogContent className="w-[95vw] max-w-[400px] sm:max-w-md max-h-[90vh] bg-white dark:bg-neutral-800 p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-base sm:text-lg font-semibold dark:text-white">
            Suggested for you (
            {suggestedUsers?.filter((user: SuggestedUser) => !user.isFollowed)
              .length || 0}
            )
          </DialogTitle>
        </DialogHeader>
        <div className="max-h-[60vh] overflow-y-auto max-sm:scrollbar-hide pr-2">
          <div className="space-y-2 sm:space-y-3">
            {suggestedUsers && suggestedUsers.length > 0 ? (
              suggestedUsers
                .filter((user: SuggestedUser) => !user.isFollowed)
                .map((user: SuggestedUser) => (
                  <div
                    key={user._id}
                    className="flex items-center justify-between hover:bg-neutral-50 dark:hover:bg-neutral-700 py-2 sm:py-3 px-2 rounded-lg transition-all duration-200"
                  >
                    <div
