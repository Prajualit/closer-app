...
const Post: React.FC<PostProps> = ({ post, onLike, onComment }) => {
  ...
  const handleLike = async () => {
    ...
    if (onLike && post._id !== undefined)
      onLike(post._id as string | number, newLikedState);
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          setNewComment("");
          if (onComment && post._id !== undefined)
            onComment(post._id as string | number, data.data.comment);
        }
      }
    } catch (error) {
  ...
  const handleShare = () => {
    ...
    if (post.media && post.media.url) {
      isVideo =
        Boolean(post.media.url.match(/\.(mp4|webm|mov)$/i)) ||
        post.media.url.includes("video");
      if (isVideo) {
        shareText = post.media.caption
          ? `Watch this video: ${post.media.caption}`
    ...
    <div className="w-full  bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-100 dark:border-neutral-700">
      ...
      <div className="p-3 sm:p-4 flex items-center justify-start space-x-2 sm:space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            {formatUploadDate(post.media?.uploadedAt)}
          </p>
        </div>
      ...
      <div className="p-3 sm:p-4">
        ...
        <div className="mb-2 flex flex-col sm:flex-row sm:space-x-2 space-y-1 sm:space-y-0">
          ...
          <span className="font-semibold text-xs sm:text-sm text-neutral-900 dark:text-white">
            {commentsCount.toLocaleString()} comment
            {commentsCount === 1 ? "" : "s"}
          </span>
        </div>
        ...
        {post.media.caption && (
          <div className="mb-2">
            <span className="font-semibold text-xs sm:text-sm mr-2 text-neutral-900 dark:text-white">
              {post.username}
            </span>
            <span className="text-xs sm:text-sm text-neutral-800 dark:text-neutral-200 break-words">
              {post.media.caption}
            </span>
          </div>
        )}
