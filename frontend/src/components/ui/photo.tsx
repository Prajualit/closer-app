...
const FavouriteIcon: React.FC<FavouriteIconProps> = ({ size = 24, color = "#000000" }) => {
    return (
        <svg
    ...
    return (
        <div className='w-[60%] '>
            <div className='p-3 flex items-center justify-start space-x-2  '>
                <Image width={32} height={32} className='rounded-full' src={userDetails?.avatarUrl || ""} alt="" />
                <h1 className='font-semibold'>{userDetails?.username || ""}</h1>
            </div>
            <Image width={100} height={100} className='w-full bg-cover bg-center ' src={userDetails?.avatarUrl || ""} alt="" />
