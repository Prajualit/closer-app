...
const Userdata: React.FC = () => {
  ...
  const photosCount = Array.isArray(userDetails?.media)
    ? userDetails.media.filter(
        (item: MediaItem) => item.resource_type === "image"
      ).length
    : 0;
  const filmsCount = Array.isArray(userDetails?.media)
    ? userDetails.media.filter(
        (item: MediaItem) => item.resource_type === "video"
      ).length
    : 0;
  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      ...
      if (
        userDetails.followersCount === undefined ||
        userDetails.followingCount === undefined
      ) {
        try {
          const response = await fetch(
            API_ENDPOINTS.USER_PROFILE(userDetails._id),
            {
              credentials: "include",
            }
          );
    ...
    <div>
      ...
      <div className="lg:hidden flex flex-col items-center space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-2">
          ...
          <h2 className="text-sm sm:text-base text-neutral-500 dark:text-neutral-400">
            @{userDetails.username}
          </h2>
          {userDetails.bio && (
