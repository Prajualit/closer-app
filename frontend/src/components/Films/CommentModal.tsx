...
const CommentModal: React.FC<CommentModalProps> = ({ isOpen, onClose, film, onCommentUpdate }) => {
    ...
    const currentUser = useSelector((state: { user: { user: User } }) => state.user.user);

    // Now define loadComments after all state/refs
    const loadComments = React.useCallback(async (reset: boolean = true) => {
