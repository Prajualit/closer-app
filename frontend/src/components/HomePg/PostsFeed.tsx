...
interface PostsFeedProps {
  suggestedUsersComponent?: ReactNode;
}

const PostsFeed: React.FC<PostsFeedProps> = ({ suggestedUsersComponent }) => {
  ...
  const LoadingSkeleton = () => (
    <div className="w-full sm:max-w-[500px] mt-6 bg-white dark:bg-neutral-800 rounded-lg shadow-sm border border-neutral-100 dark:border-neutral-700 mb-6 animate-pulse">
    ...
    async (pageNum = 1, reset = false) => {
      ...
      try {
        ...
        if (data.success && data.data.posts) {
          setPosts((prev) =>
            reset ? data.data.posts : [...prev, ...data.data.posts]
          );
  ...
  }, [fetchPosts, loading, hasMore, refreshing, page]);
  ...
  // Removed infinite scroll: load more posts only on button click

  const handleLike: (postId: string | number, liked: boolean) => void = (
    postId,
    liked
  ) => {
    ...
    <div className="w-full flex flex-col items-center bg-neutral-50 dark:bg-neutral-900 min-h-screen">
      ...
      {refreshing && (
        <div className="w-full max-w-md mb-4 text-center">
          <div className="inline-flex items-center px-4 py-2 bg-neutral-50 dark:bg-neutral-800 text-black dark:text-neutral-100 rounded-full text-sm font-medium">
            <svg
      ...
      <div className="space-y-3">
        {posts.map((post, index) => (
          <div
            key={`${post._id}-${post.media._id}-${index}`}
            className="fade-in"
          >
