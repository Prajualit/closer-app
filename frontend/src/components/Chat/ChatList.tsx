...
const ChatList: React.FC<ChatListProps> = ({
  ...
  const [internalChatRooms, setInternalChatRooms] = useState<ChatRoom[]>([]);
  const actualChatRooms = chatRooms || internalChatRooms;
  const actualSetChatRooms = setChatRooms || setInternalChatRooms;

  // Set loading to false when chatRooms are loaded
  useEffect(() => {
    if (actualChatRooms.length > 0) {
      setLoading(false);
    }
  }, [actualChatRooms.length]);

  // Listen for real-time message updates to update chat list
  useEffect(() => {
    if (socket) {
      const handleChatUpdate = (messageData: any) => {
        // Only update if the message is for a chat that exists in our list
        actualSetChatRooms((prev: ChatRoom[]) => {
          const roomExists = prev.some(
            (room) => room.chatId === messageData.chatId
          );
          if (!roomExists) {
          ...
          const updatedRooms = prev.map((room) => {
            if (room.chatId === messageData.chatId) {
              ...
              const isFromOtherUser =
                messageData.sender._id !== userDetails?._id;
              return {
                ...room,
                lastMessage: {
                ...
                unreadCount:
                  isFromOtherUser && selectedChatId !== messageData.chatId
  ...
  }, [socket, userDetails, selectedChatId, actualSetChatRooms]);
  ...
  useEffect(() => {
    if (
      ...
      !hasAutoSelected &&
      selectedChatId !== autoSelectChatId
    ) {
      const chatToSelect = actualChatRooms.find(
        (room) => room.chatId === autoSelectChatId
      );
  ...
  const searchUsersFunction = React.useCallback(async (): Promise<void> => {
    try {
      setSearchLoading(true);
      const response = await makeAuthenticatedRequest(
        API_ENDPOINTS.USER_SEARCH(searchQuery)
      );
      const data = await response.json();

      if (data.success) {
        // Filter out current user and users already in chat rooms
        const existingChatUserIds = actualChatRooms.flatMap((room: ChatRoom) =>
  ...
  }, [searchQuery, actualChatRooms, userDetails._id]);
  ...
  }, [searchQuery, searchUsersFunction]);
  ...
  const markChatAsRead = async (chatId: string): Promise<void> => {
    try {
      ...
      actualSetChatRooms((prev: ChatRoom[]) =>
        prev.map((room) =>
          room.chatId === chatId ? { ...room, unreadCount: 0 } : room
        )
      );
    } catch (error) {
  ...
  const updateChatbotLastMessage = (message: string): void => {
    actualSetChatRooms((prev: ChatRoom[]) =>
      prev.map((room) =>
        room.isChatbot
          ? {
  ...
  const handleViewProfile = (e: React.MouseEvent, userId: string): void => {
    e.stopPropagation(); // Prevent chat selection
    router.push(`/profile/${userId}`);
  };

  const startChatbotConversation = async (): Promise<void> => {
    try {
      // Get or create chatbot room from backend
      const response = await makeAuthenticatedRequest(
        API_ENDPOINTS.CHATBOT_ROOM
      );
      const data = await response.json();
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-neutral-900">
      ...
      <div className="p-3 sm:p-4 border-b border-neutral-200 dark:border-neutral-700">
        <h2 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-neutral-900 dark:text-white">
          Messages
        </h2>
      ...
      <ScrollArea className="flex-1 mt-3">
        {actualChatRooms.length > 0 ? (
          <div className="divide-y">
                ...
                >
                    ...
                    <div className="flex-1 text-left">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <p
                            className={`font-medium ${unreadCount > 0 ? "text-neutral-900 dark:text-white" : "text-neutral-700 dark:text-neutral-300"}`}
                          >
                      ...
                      <p
                        className={`text-sm truncate ${unreadCount > 0 ? "text-neutral-900 font-medium" : "text-neutral-500 dark:text-neutral-400"}`}
                      >
                        {chatRoom.lastMessage?.content ||
                          "Start a conversation..."}
                      </p>
                  ...
                  <Button
                    ...
                    onClick={(e) =>
                      ...
                      handleViewProfile(e, otherParticipant._id)
                    }
            ...
            {!searchQuery &&
              !actualChatRooms.some((room) => room.isChatbot) && (
                <div className="p-4 border-t border-neutral-100 dark:border-neutral-700">
                  <div className="bg-white dark:bg-neutral-800 rounded-[15px] shadow-md p-4">
                    ...
                    <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-3">
                      I&apos;m here to be your friend! Let&apos;s talk about
                      anything - your day, your dreams, your thoughts, or just
                      have a deep conversation like close friends do!
                    </p>
                    <LoadingButton
