...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const { socket, joinChat, sendMessage, emitTyping, emitStopTyping } =
    useSocket() as SocketContextType;
  const userDetails = useSelector(
    (state: { user: { user: Participant } }) => state.user.user
  );
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      const response = await makeAuthenticatedRequest(
        API_ENDPOINTS.CHAT_MESSAGES(chatRoom.chatId)
      );
  ...
  const handleReceiveMessage = useCallback((messageData: Message): void => {
  ...
  const handleUserStopTyping = useCallback((data: TypingUser): void => {
    setTypingUsers((prev) => prev.filter((u) => u.userId !== data.userId));
  }, []);

  const handleChatbotMessage = useCallback(
  ...
  useEffect(() => {
    if (chatRoom?.chatId) {
      console.log(
        "Joining chat and fetching messages for:",
        chatRoom.chatId,
        "isChatbot:",
        isChatbot
      );
  ...
  const handleSendMessage = async (): Promise<void> => {
    ...
    if (isChatbot) {
      ...
      try {
        ...
        setMessages((prev: Message[]) =>
          prev.filter((msg) => msg._id !== messageData._id)
        );
    ...
    <div className="flex flex-col h-full bg-white dark:bg-neutral-900">
      ...
      <div className="flex items-center justify-between p-3 sm:p-4 border-b border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800">
        <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
          ...
          {otherParticipant && (
            <div
              ...
              onClick={() =>
                !isChatbot && router.push(`/profile/${otherParticipant._id}`)
              }
            >
        ...
        <Button
          variant="ghost"
          size="sm"
          className="text-neutral-600 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700 p-1 sm:p-2 h-8 w-8 sm:h-9 sm:w-9 flex-shrink-0"
        >
          ...
          {messages.map((message, index) => {
            ...
            return (
              <div key={message._id || index}>
                ...
                >
                  <div
                    className={`flex items-end space-x-1 sm:space-x-2 max-w-[280px] sm:max-w-xs lg:max-w-md ${isOwnMessage ? "flex-row-reverse space-x-reverse" : ""}`}
                  >
                    {!isOwnMessage && showAvatar && (
                      <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full overflow-hidden flex-shrink-0">
                        <Image
                          src={
                            message.sender.avatarUrl || "/default-avatar.svg"
                          }
                    ...
                    <div
                      className={`px-3 sm:px-4 py-2 flex flex-col items-start space-y-1 shadow-lg rounded-[10px] ${
                        isOwnMessage
                    ...
                    >
                      ...
                      <p
                        className={`text-xs mt-1 ${isOwnMessage ? "text-neutral-500 dark:text-neutral-400" : "text-neutral-500 dark:text-neutral-400"}`}
                      >
            ...
            <div className="flex items-center space-x-1 sm:space-x-2">
              ...
              <div className="bg-neutral-100 dark:bg-neutral-700 px-3 sm:px-4 py-2 rounded-lg">
                <div className="flex space-x-1">
                  ...
                  <div
                    className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-neutral-400 dark:bg-neutral-500 rounded-full animate-bounce"
