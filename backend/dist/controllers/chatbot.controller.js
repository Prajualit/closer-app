...
const getOrCreateChatbotRoom = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, chatRoom, "Chatbot room retrieved successfully"));
});
...
const getChatbotMessages = asyncHandler(async (req, res) => {
    ...
    const transformedMessages = messages.reverse().map(message => {
        if (message.isAiMessage) {
            // aiSenderInfo may be a Buffer, so convert to object if needed
            let aiSenderInfo = message.aiSenderInfo;
            if (aiSenderInfo && typeof aiSenderInfo === 'object' && typeof aiSenderInfo.toString === 'function' && !aiSenderInfo.id) {
                try {
                    aiSenderInfo = JSON.parse(aiSenderInfo.toString());
                }
                catch {
                    aiSenderInfo = {};
                }
            ...
            return {
                ...
                sender: {
                    _id: aiSenderInfo.id,
                    name: aiSenderInfo.name,
                    avatarUrl: aiSenderInfo.avatarUrl
                }
            };
        }
        else {
