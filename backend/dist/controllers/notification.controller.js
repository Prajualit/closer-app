...
const markAsRead = asyncHandler(async (req, res) => {
    ...
    const userId = req.user._id;
    const notification = await Notification.findOneAndUpdate({ _id: notificationId, recipient: userId }, { read: true, readAt: new Date() }, { new: true });
    if (!notification) {
...
const getUnreadCount = asyncHandler(async (req, res) => {
    try {
        ...
        return res.status(200).json(new ApiResponse(200, { unreadCount: unreadCount || 0 }, "Unread count retrieved successfully"));
    }
    catch (error) {
...
const notifyFollow = async (followerId, followedUserId) => {
    ...
    if (!follower)
        return null;
    return createNotification(followedUserId, followerId, 'follow', `${follower.name} (@${follower.username}) started following you`, {});
};
const notifyLike = async (likerId, postOwnerId, postId) => {
    const liker = await User.findById(likerId).select('username name');
    if (!liker)
        return null;
    return createNotification(postOwnerId, likerId, 'like', `${liker.name} (@${liker.username}) liked your post`, { postId });
};
const notifyComment = async (commenterId, postOwnerId, postId, commentId) => {
    ...
    if (!commenter)
        return null;
    return createNotification(postOwnerId, commenterId, 'comment', `${commenter.name} (@${commenter.username}) commented on your post`, { postId, commentId });
};
const notifyMessage = async (senderId, recipientId, messageId, chatId) => {
    ...
    if (!sender)
        return null;
    return createNotification(recipientId, senderId, 'message', `${sender.name} (@${sender.username}) sent you a message`, { messageId, chatId });
};
export { getNotifications, markAsRead, markAllAsRead, deleteNotification, createNotification, getUnreadCount, notifyFollow, notifyLike, notifyComment, notifyMessage, };
