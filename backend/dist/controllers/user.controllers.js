...
const registerUser = asyncHandler(async (req, res) => {
    ...
    return res
        ...
        .json(new ApiResponse(200, { user: createdUser, accessToken, refreshToken }, "User registered and logged in successfully"));
});
...
const editUser = asyncHandler(async (req, res) => {
    ...
    const updateResult = await User.updateOne({ _id: req.user._id }, { $set: updateFields });
    ...
    return res
        ...
        .cookie("accessToken", accessToken, options)
        .cookie("refreshToken", refreshToken, options)
        .json(new ApiResponse(200, { user, accessToken, refreshToken }, "User updated successfully"));
});
...
const getUserProfile = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, { user: userProfile }, "User profile fetched successfully"));
});
// Follow a user
...
const getFollowStatus = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, { isFollowing }, "Follow status fetched successfully"));
});
// Get user photos
const getUserPhotos = asyncHandler(async (req, res) => {
    ...
    const photos = user.media.filter(item => item.resource_type === 'image' ||
        !item.resource_type ||
        item.url.match(/\.(jpeg|jpg|gif|png)$/i));
    return res.status(200).json(new ApiResponse(200, { photos }, "User photos fetched successfully"));
});
...
const getUserFilms = asyncHandler(async (req, res) => {
    ...
    const films = user.media.filter(item => item.resource_type === 'video' ||
        item.url.match(/\.(mp4|avi|mov|wmv|flv|webm)$/i));
    return res.status(200).json(new ApiResponse(200, { films }, "User films fetched successfully"));
});
export { loginUser, logoutUser, refreshAccessToken, registerUser, getCurrentUser, editUser, verifyPassword, changePassword, deleteUserAccount, searchUsers, getUserProfile, followUser, unfollowUser, getFollowStatus, getUserPhotos, getUserFilms, };
