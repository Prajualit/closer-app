...
const getOrCreateChatRoom = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, chatRoom, "Chat room retrieved successfully"));
});
...
const saveMessage = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(201, populatedMessage, "Message saved successfully"));
});
...
const getUserChatRooms = asyncHandler(async (req, res) => {
    ...
    const chatRoomsWithUnreadCounts = await Promise.all(chatRooms.map(async (chatRoom) => {
    ...
    return res.status(200).json(new ApiResponse(200, chatRoomsWithUnreadCounts, "Chat rooms retrieved successfully"));
});
...
export { getOrCreateChatRoom, getChatMessages, saveMessage, getUserChatRooms, markMessagesAsRead };
