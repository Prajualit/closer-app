...
const fixCloudinaryUrls = asyncHandler(async (req, res) => {
    try {
        ...
        // Fix avatar URLs
        const avatarResult = await User.updateMany({ avatarUrl: { $regex: "^http://res.cloudinary.com" } }, [{
                $set: {
                    avatarUrl: {
                        $replaceOne: {
                            input: "$avatarUrl",
        ...
        // Fix media URLs
        const mediaResult = await User.updateMany({ "media.url": { $regex: "^http://res.cloudinary.com" } }, [{
                $set: {
                    "media.$[].url": {
                        $replaceOne: {
                            input: "$media.$[].url",
