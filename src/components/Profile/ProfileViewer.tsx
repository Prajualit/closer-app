...
const ProfileViewer: React.FC<ProfileViewerProps> = ({ isOpen, onClose, userId, userProfile }) => {
    ...
    const videoRefs = React.useRef<Array<React.RefObject<HTMLVideoElement | null>>>([]);
    ...
    const currentUser = useSelector((state: { user: { user: UserType } }) => state.user.user);
    ...
    useEffect(() => {
        ...
        if (videoRefs.current.length !== films.length) {
            videoRefs.current = Array.from({ length: films.length }, () => React.createRef<HTMLVideoElement>());
        }
    ...
    const fetchUserMedia = async () => {
        ...
        try {
            ...
            if (photosResponse.ok) {
                ...
                setPhotos((photosData.data?.photos || []).map((p: any) => ({ url: p.url || '', ...p })));
            }

            if (filmsResponse.ok) {
                const filmsData = await filmsResponse.json();
                // Ensure all items are objects with url
                setFilms((filmsData.data?.films || []).map((f: any) => typeof f === 'object' && f.url ? f : { url: String(f) }));
            }
        } catch (error) {
    ...
    const SmartImage: React.FC<SmartImageProps> = ({ src, alt = "Image", containerClass = "" }) => {
                                ...
                                <>
                                    {hasPhotos ? (
                                        <div className="grid grid-cols-2 sm:grid-cols-3 items-center justify-center gap-1 sm:gap-2">
                                            {photos.map((photo, i) => {
                                                const url = typeof photo === 'object' && photo.url ? photo.url : String(photo);
                                                return (
                                                    <div
                                                        key={i}
                                ...
                                <>
                                    {hasFilms ? (
                                        <div className="grid grid-cols-2 sm:grid-cols-3 items-center justify-center gap-1 sm:gap-2">
                                            ...
                                            {films.map((film, i) => {
                                                const url = typeof film === 'object' && film.url ? film.url : String(film);
