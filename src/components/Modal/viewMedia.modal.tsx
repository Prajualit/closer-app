...
const ImageModal: React.FC<ImageModalProps> = ({ imageUrl, onClose, user, videoUrl }) => {
    ...
    useEffect(() => {
        ...
        const handleLikeUpdate = (payload: { postId: string; mediaId: string; likesCount: number; isLikedByCurrentUser: boolean }) => {
            if (payload.postId === postId && payload.mediaId === mediaId) {
                setLikesCount(payload.likesCount);
                setIsLiked(payload.isLikedByCurrentUser);
            }
        };
        const handleCommentUpdate = (payload: { postId: string; mediaId: string; commentsCount: number; comment: Comment }) => {
            if (payload.postId === postId && payload.mediaId === mediaId) {
    ...
    const handleLike = async (): Promise<void> => {
        ...
        setLikesCount((prev: number) => newLikedState ? Math.max(0, prev + 1) : Math.max(0, prev - 1));
        try {
            ...
            if (res.ok) {
                ...
                setIsLiked(!newLikedState);
                setLikesCount((prev: number) => newLikedState ? Math.max(0, prev - 1) : prev + 1);
            }
        } catch (e) {
            setIsLiked(!newLikedState);
            setLikesCount((prev: number) => newLikedState ? Math.max(0, prev - 1) : prev + 1);
    ...
    const getMediaUrl = (media: Media | string | undefined): string => {
        ...
        if (media && typeof (media as Media).url === 'string') return (media as Media).url ?? '';
        return '';
    };
