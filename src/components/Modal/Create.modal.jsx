    ...
    const MediaIcon = ({ size = 96, color = "currentColor" }) => {
        return (
            <svg
                ...
                width={size}
                height={(size * 77.3) / 96} // maintains aspect ratio
    ...
    return (
        <Dialog className="z-50 ">
            <DialogTrigger asChild>
                <button className={`transition-all duration-300 flex items-center space-x-2 rounded-[8px] px-5 py-3 hover:bg-[#efefef]  focus:bg-neutral-100 focus:text-black w-full ${activeNav === nav.name.toLowerCase() ? "text-black" : "text-neutral-500"}`}>
                    {nav.icon}
                    <span>{nav.name}</span>
                </button>
            </DialogTrigger>
            ...
            <DialogContent className="bg-white h-[80%] w-[40%] ">
                ...
                <DialogDescription className="flex flex-col space-y-5 justify-center items-center text-lg text-black">
                    <MediaIcon size={96} color="currentColor" />
